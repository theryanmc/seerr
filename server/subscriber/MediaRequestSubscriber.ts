...
export class MediaRequestSubscriber implements EntitySubscriberInterface<MediaRequest> {
  ...
  public async updateParentStatus(entity: MediaRequest): Promise<void> {
    ...
    if (
      ...
      // Season entity was never renamed to Alt (seasons don't apply to books),
      // so it still uses status4k for the alternate-version status.
      const seasonStatusKey = entity.isAlt ? 'status4k' : 'status';
